...
describe("POST /api/s3", () => {
  ...
  test("성공: 유효한 요청 시 presigned URL과 key를 반환해야 함", async () => {
    // 정상적으로 인증, 파라미터, S3 presigned URL 생성까지 모두 성공하는 경우
  ...
  test("실패: 필수 파라미터(filename)가 없는 경우 400 에러를 반환해야 함", async () => {
    // filename 등 필수 파라미터가 없을 때 400 반환
    const req = createMockRequest({
      method: "POST",
      headers: { authorization: "Bearer mock-token" },
      body: { contentType: mockContentType, size: mockSize },
    });
    const response = await POST(req);
    const body = await response.json();
  ...
  test("실패: 허용되지 않는 파일 타입인 경우 400 에러를 반환해야 함", async () => {
    // 허용되지 않는 파일 타입일 때 400 반환
    const req = createMockRequest({
      method: "POST",
      headers: { authorization: "Bearer mock-token" },
      body: { filename: "test.txt", contentType: "text/plain", size: mockSize },
    });
    const response = await POST(req);
    const body = await response.json();
    expect(response.status).toBe(400);
